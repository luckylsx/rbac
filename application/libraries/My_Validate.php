 ...
 * 如数组 ： $arr = ['title'=>'','name'=>''];
 * 验证规则 ：
 * 1、$rules = [['title','require'],['name','require']];
 * 2、[['title|标题','require|length:3'],['name|名称','require']]
 * $this->validator = new Validator($rules);
 * 执行验证方法：$this->validator->validate($arr);
 * 获取第一条验证错误信息：$this->validator->getError()
 * 获取全部验证错误信息：$this->validator->getAllErrors()
 */
class My_Validate{
	...
	/**
     * 验证规则
     * 如：[['title|标题','require|length:3'],['name|名称','require']]
...
*/
    ...
    */
    public static function confirm($rule, $data) {
        return isset((new Validator())->data[$rule]) && $data == (new Validator())->data[$rule];
    }
